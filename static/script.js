document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupTranslateFunction() {
        ...
        // 为所有提示词输入框添加三个空格检测
        const promptInputs = [
            promptNanoBananaInput,
            promptChatGPTInput,
            promptPositiveInput,
            promptNegativeInput
        ].filter(input => input); // 过滤掉null值

        promptInputs.forEach(input => {
            input.addEventListener('input', (e) => {
                checkTripleSpaceTrigger(e.target);
            });
            
            input.addEventListener('keydown', (e) => {
                // 监听三个空格输入
                handleTripleSpaceInput(e.target, e);
            });
        });
    }
    ...
    async function translateText(button) {
        ...
        if (!inputElement) {
            showTranslateError('找不到目标输入框');
            return;
        }

        const text = inputElement.value.trim();
        if (!text) {
            showTranslateError('请先输入要翻译的文本');
            return;
        }
        ...
        try {
            ...
            // 显示成功提示
            showTranslateSuccess(`翻译成功：${data.from} → ${data.to}`);
            
        } catch (error) {
            console.error('翻译失败:', error);
            showTranslateError(error.message || '翻译服务暂时不可用，请检查网络连接');
        } finally {
            // 恢复按钮状态
    ...
    // 检测三个空格触发
    function checkTripleSpaceTrigger(input) {
    ...
    function handleTripleSpaceInput(input, event) {
    ...
    // 显示翻译成功提示
    function showTranslateSuccess(message) {
    ...
    function showToast(message, type = 'info') {
        ...
        toast.style.cssText = `
            ...
            animation: translateToastSlideIn 0.3s ease-out;
            max-width: 300px;
            word-wrap: break-word;
        `;
        
        // 添加动画样式
        if (!document.querySelector('#translate-toast-styles')) {
            const style = document.createElement('style');
            style.id = 'translate-toast-styles';
            style.textContent = `
                @keyframes translateToastSlideIn {
                    from {
                ...
                @keyframes translateToastSlideOut {
                    from {
                        transform: translateX(0);
                        opacity: 1;
                    }
        ...
        setTimeout(() => {
            if (toast.parentNode) {
                toast.style.animation = 'translateToastSlideOut 0.3s ease-in forwards';
                setTimeout(() => {
                    if (toast.parentNode) {
